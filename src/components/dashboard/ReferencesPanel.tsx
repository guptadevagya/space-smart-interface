...
const ReferencesPanel: React.FC<ReferencesPanelProps> = ({
  ...
  const baseRows = [
    ...
    { key: 'fgrPrevalence', label: 'FGR Prevalence', format: (v: number) => (v * 100).toFixed(1) + '%' },
    { key: 'currentDetectionRate', label: 'Current Detection Rate', format: (v: number) => (v * 100).toFixed(0) + '%' },
    { key: 'oxailisDetectionRate', label: 'OxNNet Detection Rate', format: (v: number) => (v * 100).toFixed(0) + '%' },
    { key: 'cSectionCost', label: 'C-Section Cost', format: formatCurrency },
    { key: 'nicuDailyCost', label: 'NICU Daily Cost', format: formatCurrency },
    { key: 'malpracticeClaimCost', label: isUS ? 'CP Verdict (Median)' : 'Litigation Cost', format: formatCurrency },
    { key: 'hypoxicEventRate', label: 'Hypoxic Event Rate', format: (v: number) => (v * 100).toFixed(2) + '%' },
  ];
  ...
  const usExtraRows = [
    ...
    { key: 'litigationSuccessRate', label: 'Litigation Success Rate', format: (v: number) => (v * 100).toFixed(0) + '%' },
    { key: 'combinedTestRate', label: 'Screening Uptake', format: (v: number) => (v * 100).toFixed(1) + '%' },
    { key: 'emergencyCSectionRateUndiagnosed', label: 'Emergency C-Section Rate', format: (v: number) => (v * 100).toFixed(0) + '%' },
  ];

  const parameterRows = isUS ? [...baseRows, ...usExtraRows] : baseRows;

  const filledRefsCount = parameterRows.filter(
    (r) => inputs.inputReferences[r.key]?.trim()
  ).length;
            ...
            <CollapsibleTrigger asChild>
              <CardHeader className="cursor-pointer hover:bg-muted/30 transition-colors py-4">
                <div className="flex items-center justify-between">
                  ...
                  <ChevronDown className={cn('h-4 w-4 text-muted-foreground transition-transform', assumptionsOpen && 'rotate-180')} />
                </div>
                {!assumptionsOpen && (
            ...
            <CollapsibleContent>
              <CardContent className="p-0">
                ...
                <div className="px-5 py-2 bg-muted/30 border-b border-border grid grid-cols-[1fr_1fr] gap-4">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Parameter</p>
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Source / Citation</p>
                </div>
                <div className="divide-y divide-border">
                  {parameterRows.map((row) => {
                    ...
                    return (
                      <div key={row.key} className="px-5 py-3 grid grid-cols-[1fr_1fr] gap-4 hover:bg-muted/20 transition-colors group items-start">
                ...
                <div className="divide-y divide-border rounded-lg border border-border overflow-hidden">
                  {filteredBibliography.length === 0 ? (
                    ...
                    filteredBibliography.map(({ entry, index }) => {
                      ...
                      return (
                        <div key={index} className="px-4 py-3 flex items-start gap-3 hover:bg-muted/20 transition-colors group">
                          ...
                          <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button onClick={() => onBibRemove(index)}
