...
const ReferencesPanel: React.FC<ReferencesPanelProps> = ({
  inputs, onReferenceChange, onBibUpdate, onBibRemove, onBibAdd, formatCurrency, formatNumber
}) => {
  ...
  const parameterRows = [
    ...
    { key: 'malpracticeClaimCost', label: 'Litigation Cost', format: formatCurrency },
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-border">
                {parameterRows.map((row) => (
                  <div key={row.key} className="px-5 py-3 flex items-center gap-4 hover:bg-muted/20 transition-colors group">
      ...
      <Collapsible open={refsOpen} onOpenChange={setRefsOpen}>
        <Card>
          <CollapsibleTrigger asChild>
            <CardHeader className="cursor-pointer hover:bg-muted/30 transition-colors pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm font-bold">References ({inputs.bibliography.length})</CardTitle>
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-border">
                {inputs.bibliography.map((entry, i) => (
                  <div key={i} className="px-5 py-2.5 flex items-start gap-3 hover:bg-muted/20 transition-colors group">
                    <span className="text-[10px] font-bold text-muted-foreground mt-1 shrink-0">[{i + 1}]</span>
                    <input
                      type="text"
              ...
              <div className="px-5 py-3 border-t border-border">
                <button onClick={onBibAdd} className="text-xs font-semibold text-primary hover:text-primary/80 transition-colors">
                  + Add Reference
                </button>
              </div>
            </CardContent>
