...
import { BookOpen, RotateCcw, X, ChevronDown, Save, Plus, Trash2, Pencil } from 'lucide-react';
...
const InputGroup: React.FC<InputGroupProps> = ({ title, children, defaultOpen = true, onAddParameter }) => {
    ...
    <Collapsible open={open} onOpenChange={setOpen} className="border border-border rounded-lg overflow-hidden">
      <CollapsibleTrigger className="flex items-center justify-between w-full px-4 py-3 bg-muted/50 hover:bg-muted transition-colors">
        ...
        <div className="flex items-center gap-1">
          {onAddParameter && (
            <button
              onClick={(e) => { e.stopPropagation(); onAddParameter(); }}
...
const SliderField: React.FC<SliderFieldProps> = ({
  label, value, onChange, min, max, step, prefix, suffix, tooltip, isDefault = true, formatDisplay, onDelete, onEditReference
}) => {
          ...
          <Popover open={editingRef} onOpenChange={setEditingRef}>
            ...
            <PopoverContent side="right" className="max-w-[280px] text-xs p-3">
              ...
              <input
                ...
                onBlur={() => { onEditReference?.(refValue); setEditingRef(false); }}
                onKeyDown={(e) => { if (e.key === 'Enter') { onEditReference?.(refValue); setEditingRef(false); } }}
...
const AddParameterDialog: React.FC<{
  ...
  const handleAdd = () => {
    ...
    onAdd({ id, label: label.trim(), value, min, max, step, format, group, reference });
    setLabel(''); setValue(0); setMin(0); setMax(1000); setStep(1); setFormat('number'); setReference('');
    setOpen(false);
  };
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-3">
          <div>
            ...
            <Input value={label} onChange={e => setLabel(e.target.value)} className="h-8 text-xs" placeholder="e.g. Readmission Rate" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <Input type="number" value={value} onChange={e => setValue(parseFloat(e.target.value) || 0)} className="h-8 text-xs" />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2">
            <div>
              <Label className="text-xs">Min</Label>
              <Input type="number" value={min} onChange={e => setMin(parseFloat(e.target.value) || 0)} className="h-8 text-xs" />
            </div>
            <div>
              <Label className="text-xs">Max</Label>
              <Input type="number" value={max} onChange={e => setMax(parseFloat(e.target.value) || 0)} className="h-8 text-xs" />
            </div>
            <div>
              <Label className="text-xs">Step</Label>
              <Input type="number" value={step} onChange={e => setStep(parseFloat(e.target.value) || 1)} className="h-8 text-xs" />
            </div>
          </div>
          <div>
            <Label className="text-xs">Reference / Source</Label>
            <Input value={reference} onChange={e => setReference(e.target.value)} className="h-8 text-xs" placeholder="Enter citation..." />
          </div>
...
const InputSidebar: React.FC<InputSidebarProps> = ({ inputs, setInputs, isOpen, toggleSidebar, onSave, customParameters, setCustomParameters }) => {
  ...
  const handleReferenceChange = (key: string, value: string) => {
    setInputs(prev => ({ ...prev, inputReferences: { ...prev.inputReferences, [key]: value } }));
  };
      ...
      <aside className={cn(
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          <InputGroup title="Hospital Profile" onAddParameter={() => {}}>
            ...
            <AddParameterDialog group="Hospital Profile" currPrefix={currPrefix} onAdd={addCustomParam} />
          </InputGroup>
          ...
          <InputGroup title="Clinical Assumptions" onAddParameter={() => {}}>
            ...
            <AddParameterDialog group="Clinical Assumptions" currPrefix={currPrefix} onAdd={addCustomParam} />
          </InputGroup>
          ...
          <InputGroup title="Cost Parameters" onAddParameter={() => {}}>
            {inputs.region === 'US' ? (
              ...
              <>
                ...
                <SliderField label="NND Litigation" value={inputs.neonatalDeathLitigationCost} onChange={(v) => update('neonatalDeathLitigationCost', v)}
                  ...
                  onEditReference={(ref) => handleReferenceChange('neonatalDeathLitigationCost', ref)} />
            ...
            <AddParameterDialog group="Cost Parameters" currPrefix={currPrefix} onAdd={addCustomParam} />
          </InputGroup>
          ...
          <InputGroup title="Risk Ratios" onAddParameter={() => {}}>
            {inputs.region === 'US' && (
              <>
                <SliderField label="Emergency C-Section Rate" value={inputs.emergencyCSectionRateUndiagnosed} onChange={(v) => update('emergencyCSectionRateUndiagnosed', v)}
                  ...
                  onEditReference={(ref) => handleReferenceChange('emergencyCSectionRateUndiagnosed', ref)} />
            ...
            <AddParameterDialog group="Risk Ratios" currPrefix={currPrefix} onAdd={addCustomParam} />
          </InputGroup>
        </div>
